...
If Render does not provide a Java runtime in your account, deploy the backend as a Docker service.

### Files added for Docker deploy
...
2.5 Set **Root Directory** to `backend`.
...
For frontend deployments (e.g. Vercel), you can copy `frontend/.env.example` and set `VITE_API_BASE_URL` accordingly.

## Contribution workflow note

When using automated agents for repository changes, ensure each change is finalized with:
1. a git commit on the active branch, and
2. a created pull request that includes a clear title/body summary.

This avoids orphan commits that are not attached to any reviewable PR.

For traceability, include the PR title in the final status message so reviewers can find it quickly.
